...
function _placeModeMenu() {
  // Anchor to whichever trigger is on screen: the sidebar card (desktop) or the
  // Settings entry (mobile — the side cards are hidden there).
  const d1 = document.getElementById('modeDisplay');
  const vis = el => el && el.getBoundingClientRect().width > 0;
  const display = vis(d1) ? d1 : (document.getElementById('setModeBtn') || d1);
  const menu    = document.getElementById('modeMenu');
